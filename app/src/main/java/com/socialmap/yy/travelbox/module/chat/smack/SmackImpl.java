package com.socialmap.yy.travelbox.module.chat.smack;
...
import com.socialmap.yy.travelbox.module.chat.exception.XXException;
...
public class SmackImpl implements Smack {
	...
	public void sendOfflineMessages() {
		...
		mark_sent.put(ChatProvider.ChatConstants.DELIVERY_STATUS,
				ChatProvider.ChatConstants.DS_SENT_OR_READ);
	...
	private void updateRosterEntryInDB(final RosterEntry entry) {
		...
		if (mContentResolver.update(RosterProvider.CONTENT_URI, values,
				RosterProvider.RosterConstants.JID + " = ?", new String[] { entry.getUser() }) == 0)// 如果数据库无此好友
	...
	private void initServiceDiscovery() {
		...
		dm.registerReceiptReceivedListener(new DeliveryReceiptManager.ReceiptReceivedListener() {
			public void onReceiptReceived(String fromJid, String toJid,
				...
				changeMessageDeliveryStatus(receiptId, ChatProvider.ChatConstants.DS_ACKED);// 标记为对方已读，实际上遇到了点问题，所以其实没有用上此状态
			}
		});
	}
	...
	public void sendMessage(String toJID, String message) {// 发送消息
		...
		newMessage.addExtension(new DeliveryReceiptRequest());
		if (isAuthenticated()) {
			addChatMessageToDB(ChatProvider.ChatConstants.OUTGOING, toJID, message,
...
//		setStatusOffline();
	...
	public void setStatusOffline() {
		ContentValues values = new ContentValues();
		values.put(RosterProvider.RosterConstants.STATUS_MODE, StatusMode.offline.ordinal());
