...
import com.socialmap.yy.travelbox.module.chat.util.TimeUtil;
import com.socialmap.yy.travelbox.module.chat.util.XMPPHelper;
import com.socialmap.yy.travelbox.module.chat.view.CustomDialog;


public class RecentChatAdapter extends SimpleCursorAdapter {
	private static final String SELECT = ChatProvider.ChatConstants.DATE
			+ " in (select max(" + ChatProvider.ChatConstants.DATE + ") from "
			+ ChatProvider.TABLE_NAME + " group by " + ChatProvider.ChatConstants.JID
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		String selection = ChatProvider.ChatConstants.JID + " = '" + jid + "' AND "
		...
		if (msgcursor.getInt(0) > 0) {
			...
			viewHolder.dataView.setText(TimeUtil.getChatTime(msgcursor
					.getLong(msgcursor.getColumnIndex(ChatProvider.ChatConstants.DATE))));
